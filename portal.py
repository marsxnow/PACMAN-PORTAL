...
class Portal(Char):
    ...
    def move(self):
        if self.direction == 'left':
            if self.rect.centerx >= self.to_node.x:
                self.rect.centerx -= self.speed
            else:
                self.current_node = self.to_node
                self.open = True
            ...
            if self.rect.centery >= self.to_node.y:
    ...
    def fire(self):
        ...
        if self.map.valid_move(self.current_node, self.direction):
            adj_list = self.map.get_adj_path(self.current_node, self.game.last_key)
            self.next_node = adj_list[len(adj_list) - 1]

        self.to_node = self.current_node
